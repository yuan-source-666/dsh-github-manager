...
import { githubListAll, githubRequest, webLink } from "./github-client.js";
...
export function registerBranchFileTools(ctx, identity) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            const res = await githubRequest(identity, 'GET', path);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            assertRepo(ref);
            if (identity.dryRun)
                return { path: args.path, sha: '(dry-run)', commitSha: '(dry-run)', dryRun: true };
            const body = {
            ...
            if (args.sha)
                body.sha = args.sha;
            const res = await githubRequest(identity, 'PUT', `/repos/${ref.owner}/${ref.repo}/contents/${encodeURIComponent(args.path).replace(/%2F/g, '/')}`, { body });
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            assertRepo(ref);
            if (identity.dryRun)
                return { path: args.path, commitSha: '(dry-run)', dryRun: true };
            const body = { message: args.message, sha: args.sha };
            if (args.branch)
                body.branch = args.branch;
            const res = await githubRequest(identity, 'DELETE', `/repos/${ref.owner}/${ref.repo}/contents/${encodeURIComponent(args.path).replace(/%2F/g, '/')}`, { body });
