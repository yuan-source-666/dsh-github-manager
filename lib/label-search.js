...
export function registerLabelSearchTools(ctx, identity) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            assertRepo(ref);
            if (identity.dryRun)
                return { name: args.name, color: args.color ?? 'ffffff', url: '(dry-run)', dryRun: true };
            const body = { name: args.name, color: args.color ?? 'ffffff', description: args.description ?? '' };
            const res = await githubRequest(identity, 'POST', `/repos/${ref.owner}/${ref.repo}/labels`, { body });
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            const res = await githubRequest(identity, 'GET', `/search/code?q=${encodeURIComponent(args.query)}&per_page=${perPage}`);
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            const perPage = clampInt(args.perPage, 20, 1, 100);
            const res = await githubRequest(identity, 'GET', `/search/issues?q=${encodeURIComponent(args.query)}&per_page=${perPage}`);
