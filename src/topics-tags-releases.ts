...
/** Normalized release payload returned by GitHub. */
...
function toReleaseSummary(r: ReleaseSummary): {
  id: number
  tagName: string
  name: string
  draft: boolean
 ...
 */
function normalizeTopic(name: string): string | null {
  const cleaned = name
    .toLowerCase()
    .trim()
    .replace(/[ _]+/g, '-')
...
export function registerTopicTagReleaseTools(ctx: Context, identity: GitHubIdentity): void {
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{ type: 'text', text: (value.topics ?? []).length > 0 ? value.count + ' topics: ' + (value.topics as string[]).join(', ') : 'This repository has no topics.' }],
    },
    async execute(args) {
      const ref = args as RepoRef
      assertRepo(ref)
      const res = await githubRequest<{ names: string[] }>(identity, 'GET', `/repos/${ref.owner}/${ref.repo}/topics`)
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => [{ type: 'text', text: (value.dryRun ? 'Would set ' : 'Topics set to ') + value.count + ' topics: ' + (value.topics as string[]).join(', ') + (value.dryRun ? '. Dry-run.' : '') }],
    },
    async execute(args) {
      const ref = args as RepoRef
      assertRepo(ref)
      const names = [...new Set((args.topics ?? []).map(normalizeTopic).filter((n): n is string => n !== null))]
      if (names.length > 20) throw new Error('GitHub accepts at most 20 topics per repository')
      if (identity.dryRun) {
        return { topics: names, count: names.length, dryRun: true }
      }
      const res = await githubRequest<{ names: string[] }>(identity, 'PUT', `/repos/${ref.owner}/${ref.repo}/topics`, { body: { names } })
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const list = await githubListAll<{ name: string; commit: { sha: string } }>(identity, path, perPage)
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const list = await githubListAll<ReleaseSummary>(identity, path, perPage)
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const ref = args as RepoRef
      assertRepo(ref)
      const res = await githubRequest<ReleaseSummary>(identity, 'GET', `/repos/${ref.owner}/${ref.repo}/releases/latest`)
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const ref = args as RepoRef
      assertRepo(ref)
      if (!args.tagName) throw new Error('tagName is required')
      const body = {
      ...
      const res = await githubRequest<{ id: number; tag_name: string; html_url: string }>(identity, 'POST', `/repos/${ref.owner}/${ref.repo}/releases`, { body })
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      const ref = args as RepoRef
      assertRepo(ref)
      if (!Number.isFinite(args.releaseId) || args.releaseId <= 0) throw new Error('releaseId must be a positive integer')
      const body: Record<string, unknown> = {}
      if (args.tagName !== undefined) body.tag_name = args.tagName
      ...
      if (args.commitish !== undefined) body.target_commitish = args.commitish
      ...
      const res = await githubRequest<{ id: number; tag_name: string; html_url: string }>(identity, 'PATCH', `/repos/${ref.owner}/${ref.repo}/releases/${args.releaseId}`, { body })
      return { id: res.body.id, tagName: res.body.tag_name, url: res.body.html_url, dryRun: false }
    },
